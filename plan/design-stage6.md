...
**要確認（定義ファイルの誤り？）**: `data/skills/10.json` の `skill2`（ミサイル）の notes に「フルバースト時の攻撃力▲（維持型）は Stage 6」とあるが、ラピの skill2 の説明文は「最終攻撃力が最も高い敵 1 機に最終攻撃力の 528.97% のダメージ・挑発」で攻撃力▲が出てこない。Stage 6 の実装時に説明文を読み直して notes を直す（`checkedAt` も更新する）。
...
5. **`key` を作る**。`fullBurst` と全枠の `BuffTotals` の 6 フィールドを固定桁で並べた文字列。
...
2. **離散化誤差**: グループごとの `|sim.triggers − calc.triggers|` と、枠・編成の総ダメージの相対差。Stage 5 の許容値（枠 5%・編成 3%・18,000 秒で 0.5%）を据え置き、**グループ単位の実測値を verification.md に記録する**。区間が細かくなる分、グループごとの相対差は Stage 5 より大きくなるはず（9 節）。
...
5. `data/skills/` の 4 体（10 / 93 / 290 / 870）に `timed` を書き、271 の notes を直す + `definitions.test.ts` → `npm test`。`checkedAt` を更新。
...
- **区間が細かくなるほど calc の離散化誤差が効く**: calc は各区間を「秒間トリガー数 × 秒数」で置くので、区間が 10 秒 → 5 秒と短くなると、1 マガジンが 10 秒級の SR / RL ではその区間の相対誤差が倍増する。**総ダメージでは誤差が打ち消し合う**はずだが、そうならない場合は Stage 5 の 8 節 4 の代替案（窓内のトリガー数を周期から数え上げる補助関数）へ切り替える。6.4 の 2 でグループ単位の実測値を先に見る。
