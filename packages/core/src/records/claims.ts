...
export function observationIdsIn(text: string): string[] {
  ...
  for (const m of text.matchAll(OBSERVATION_RANGE)) {
    for (let n = Number(m[2]); n <= Number(m[3]); n++) ids.push(`${m[1]}-${String(n).padStart(2, '0')}`);
...
export function validateClaims(claims: readonly Claim[], observationIds: ReadonlySet<string>): string[] {
  ...
  claims.forEach((c, i) => {
    const expected = `C-${String(i + 1).padStart(4, '0')}`;
    if (c.id !== expected) errors.push(`${c.id}: ID は C-0001 から通し番号（${i + 1} 行目は ${expected}）`);
