  ...
  return (
    <section className="panel result breakdown-panel">
      ...
      <p className="scope">
        ...
        N
        機」（発動の瞬間の順位）と、回復・「回復効果が適用された時」の発動（回復は定義のあるニケによるものだけ）にも対応します。通常攻撃が命中するたびの追加ダメージ、効果のあるスタック（最大装弾数▼など）、「自分が命中率増加状態なら」、装弾数無限と使用武器の変更（殲滅モード）も扱います（命中率は条件の判定にだけ使います）。防御力▼などの敵デバフ・ヒット率・被弾・OL・キューブは含みません。定義のないニケはスキルなしで計算します。SG
        は全ペレット命中が前提です。
      </p>
    </section>
  );
