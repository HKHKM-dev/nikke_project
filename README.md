...
- `packages/core/data` は Blablalink 公開 CDN から取得したデータを正規化したものです。装備の強化 Lv 別の値（`data/masters/gear.json` の Lv1〜5）と OL オプションの Lv 別の上昇値（`data/masters/overload.json`）は CDN に無いため、コミュニティの検証値を転記しています（装備の Lv0 は CDN、T9 Lv5 の合計は射撃場の実測で確認。OL はオプションの種類だけ CDN と照合し、数値は未検証）。
